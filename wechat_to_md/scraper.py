"""Browser page fetching with retry logic and verification-page detection."""
...
# Indicators that WeChat is showing a verification page
_CAPTCHA_INDICATORS = [
    "js_verify",
    "verify_container",
    "环境异常",
...
def _is_captcha_page(html: str) -> bool:
    """Check if the HTML contains verification-page indicators."""
...
async def fetch_page_html(
    ...
    backoff on network/timeout errors. Verification pages are not retried.
    """
    logger = get_logger()
        ...
        try:
            async with AsyncCamoufox(headless=headless) as browser:
                ...
                if _is_captcha_page(html):
                    raise CaptchaError(
                        "WeChat verification page detected. "
                        "Try running with --no-headless for manual confirmation."
                    )
            ...
            raise  # Do not retry verification pages
